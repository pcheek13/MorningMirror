...
- Settings cards are collapsed by default so you only open the section you want to edit.
- The settings button hides itself while sleeping to leave only the half-moon wake control visible.
- A restart control lives inside the settings tray and calls the configurable `rebootCommand` (defaults to `sudo /sbin/reboot`).
